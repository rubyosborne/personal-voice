...
# Hand off to the skill's Job 4 via headless Claude Code.
claude -p "Run the voice skill's Job 4 — INGEST INBOX now. Process every queued \
inbox/*.md file (ignore inbox/.gitkeep and inbox/README.md): save each into the \
correct voice's corpus with the standard dated header, re-distill that voice's \
STYLE.md, promote any cross-voice trait to SHARED.md, delete each ingested inbox \
file, then make ONE commit and push. Skip (don't delete) any file with a \
missing/invalid voice and report it. If the inbox has no real samples, do \
nothing." \
